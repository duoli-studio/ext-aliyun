...
use App\Jobs\Platform\StaticsBaoziOrderNum;
use App\Lemon\Dailian\Action\ActionPlatformOrder;
use App\Lemon\Dailian\Application\Platform\Request\BaoziReq;
...
class Baozi implements Platform
{
	...
	const SYNC_PLATFORM_BAOZI = 'baozi';
	...
	private $platform = PlatformAccount::PLATFORM_BAOZI;
	...
	public function __construct($pt_account_id, $platformOrder, $handle_pam = null)
	{
		...
		$this->checkEnv();
		$this->req        = new BaoziReq($pt_account_id);
	...
	public function publish()
	{
		...
		// \Log::debug($param);
		// 如果服务器编码为6位传server_code 如果为12位传server_code plat_dianjingbaozi弃用
		if (strlen(GameServer::baoziServerCode($this->order->server_id)) == 6) {
			// $param['plat_dianjingbaozi'] = GameServer::baoziServerCode($this->order->server_id);
			$param['server_code'] = GameServer::baoziServerCode($this->order->server_id);
		} else if (strlen(GameServer::baoziServerCode($this->order->server_id)) == 12) {
			$param['server_code'] = GameServer::baoziServerCode($this->order->server_id);
		}

		if (!isset($param['plat_dianjingbaozi']) && !isset($param['server_code'])) {
		...
		if ($resp['status'] == 0) {
			$success = '发布到电竞包子成功';
			// 更新 订单号
			$this->status->baozi_order_no   = $resp['data']['order_no'];
			$this->status->baozi_pt_message = $success;
			$this->status->baozi_is_error   = 0;
			$this->status->baozi_is_delete  = 0;
			$this->status->baozi_is_publish = 1;
			$this->status->save();
			...
			// 记录同步订单错误
			$this->status->baozi_is_delete  = 0;
			$this->status->baozi_is_publish = 0;
			$this->status->baozi_is_error   = 1;
			$this->status->baozi_pt_message = $error;
			$this->status->save();
	...
	public function syncDetail()
	{
		...
		if ($result['status'] == 0) {
			...
			if (isset($result['data']['assigned_at'])) {
				$this->status->baozi_assigned_at = $result['data']['assigned_at'];
				$this->status->baozi_is_accept   = $result['data']['sd_account_id'] ? 1 : 0;
				$this->status->baozi_order_no    = $result['data']['order_no'];
				$this->status->baozi_sd_uid      = $result['data']['sd_account_id'];
				// left_hour
				$hourDiff = Carbon::createFromFormat('Y-m-d H:i:s', $result['data']['assigned_at'])->diffInHours(Carbon::now());

				$this->status->baozi_left_hour = (($result['data']['order_hours'] - $hourDiff) > 0) ? $result['data']['order_hours'] - $hourDiff : 0;

				/** @type PamAccount $user */
				$this->status->baozi_sd_username     = $result['data']['sd_account_name'];
				$this->status->baozi_sd_mobile       = $result['data']['sd_mobile'];
				$this->status->baozi_sd_qq           = $result['data']['sd_qq'];
				$this->status->baozi_order_add_price = $result['data']['order_add_price'];
			}

			if (isset($result['data']['ended_at'])) {
				$this->status->baozi_is_over  = 1;
				$this->status->baozi_ended_at = $result['data']['ended_at'];
			}
			/* 删除订单状态同步 (Mark Zhao) ----------- */
			if (isset($result['data']['order_status']) && $result['data']['order_status'] == self::ORDER_STATUS_DELETE) {
				$this->status->baozi_is_delete  = 1;
				$this->status->baozi_pt_message = '订单已删除!';
			...
			if (isset($result['data']['publisher_id'])) {
				$publisher = PlatformAccount::where('baozi_userid', $result['data']['publisher_id'])->first();
			}

			if (!isset($publisher)) {
				$this->status->baozi_pub_username = '';
			} else {
				$this->status->baozi_pub_uid      = $publisher->baozi_userid;
				$this->status->baozi_pub_username = $publisher->baozi_nickname;
				$this->status->baozi_pub_mobile   = $publisher->mobile;
				$this->status->baozi_pub_qq       = $publisher->qq;
				$this->status->baozi_pub_contact  = $publisher->contact;
			}

			$this->status->baozi_is_star          = $result['data']['is_pub_star'] == 'Y' ? 1 : 0;
			$this->status->baozi_cancel_type      = $result['data']['cancel_type'];
			$this->status->baozi_kf_status        = $result['data']['kf_status'];
			$this->status->baozi_order_price      = $result['data']['order_price'];
			$this->status->baozi_order_hour       = $result['data']['order_hours'];
			$this->status->baozi_exception_status = $result['data']['exception_status'];
			$this->status->baozi_exception_type   = $result['data']['exception_type'];
			$this->status->baozi_is_public        = $result['data']['is_public'] == 'Y' ? 1 : 0;
			$this->status->baozi_cancel_status    = $result['data']['cancel_status'];
			$this->status->baozi_order_status     = $result['data']['order_status'];
			$this->status->baozi_is_exception     = $result['data']['is_exception'] == 'Y' ? 1 : 0;
			$this->status->baozi_is_lock          = $result['data']['order_lock'] == 'Y' ? 1 : 0;
			$this->status->baozi_pub_pay          = $result['data']['pub_pay'];
			$this->status->baozi_sd_pay           = $result['data']['sd_pay'];
			$this->status->baozi_created_at       = $result['data']['created_at'];
			$this->status->baozi_overed_at        = $result['data']['overed_at'];
			$this->status->baozi_ended_at         = $result['data']['ended_at'];
			$this->status->baozi_order_add_price  = $result['data']['order_add_price'];
			$this->status->sync_at                = Carbon::now();


			if (in_array($this->status->baozi_order_status, [self::ORDER_STATUS_QUASH]) ||
				in_array($this->status->baozi_order_status, [self::ORDER_STATUS_ING]) ||
				in_array($this->status->baozi_order_status, [self::ORDER_STATUS_EXCEPTION]) ||
				in_array($this->status->baozi_order_status, [self::ORDER_STATUS_OVER]) ||
				in_array($this->status->baozi_order_status, [self::ORDER_STATUS_EXCEPTION]) ||
				in_array($this->status->baozi_order_status, [self::ORDER_STATUS_CANCEL])
			) {
				// 已经接手
				$this->status->baozi_is_accept = 1;
				$this->status->accepted_at     = $this->status->baozi_assigned_at;
			}
			$this->status->save();
			$this->order->save();
			// 同步到平台主订单
		...
		/*
			...
			$this->status->baozi_is_publish = 1;
			$this->status->baozi_is_delete  = 1;
			$this->status->baozi_order_no   = '';
			$this->status->baozi_pt_message = $result['message'];
			$this->status->sync_at       = Carbon::now();
			$this->status->save();
	...
	{
		$param = [
			'order_no' => $this->status->baozi_order_no,
		];
	...
	public function progress($note, $picture = null)
	{
		...
		if ($picture) {
			...
			// $expiration      = $oss['data']['expiration'];    //过期时间
			...
			$url = $endpoint . '/' . $directory . $time . $imageName . '!wm';
		}
		if (!$url) {
	...
	/**
	 * 临时上传授权key
	 * "directory": "up/201709/10281/081112/",
	 * "prefix_url": "http://temp_key.oss-cn-qingdao.aliyuncs.com",
	 * "bucket": "temp_key",
	 * "access_key_secret": "HEYHS6srUgF7KZhA5e...PYY7XHJ5qdTaskQ",
	 * "access_key_id": "STS.HzQc1...NgzQq",
	 * "expiration": "2017-09-08T04:12:08Z",
	 * "security_token": "CaKjyaEp4...DfJovM="
	 */
	...
	{
		if (!$this->status->baozi_order_no) {
			return $this->setError('[获取详情]本地不存在代练订单号， 无法获取详情');
		}
	...
	{
		dispatch(new StaticsBaoziOrderNum($this->platformAccount, $this->platform));
	}
	...
	{
		if ($this->status->baozi_is_publish && $this->status->baozi_is_delete) {
			return true;
		} else {
			return false;
		}
	...
	{
		return $this->status->baozi_is_publish;
	}
	...
	private function syncToPlatformOrder()
	{
		...
		$this->order->order_status      = $this->status->baozi_order_status;
		$this->order->is_exception      = $this->status->baozi_is_exception;
		$this->order->cancel_status     = $this->status->baozi_cancel_status;
		$this->order->cancel_type       = $this->status->baozi_cancel_type;
		$this->order->kf_status         = $this->status->baozi_kf_status;
		$this->order->assigned_at       = $this->status->baozi_assigned_at;
		$this->order->overed_at         = $this->status->baozi_overed_at;
		$this->order->order_lock        = $this->status->baozi_is_lock;
		$this->order->sd_uid            = $this->status->baozi_sd_uid;
		$this->order->sd_username       = $this->status->baozi_sd_username;
		$this->order->sd_qq             = $this->status->baozi_sd_qq;
		$this->order->sd_mobile         = $this->status->baozi_sd_mobile;
		$this->order->order_price       = $this->status->baozi_order_price;
		$this->order->order_hours       = $this->status->baozi_order_hour;
		$this->order->is_star           = $this->status->baozi_is_star;
