...
use App\Jobs\Platform\StaticsMaoOrderNum;
...
class Mao implements Platform
{
	...
	const MAO_LOCK                = 20002;      //锁定账号
	const MAO_UNLOCK              = 20010;      //取消锁定
	const MAO_CHECK               = 20005;      //申请验收
	const MAO_CHECK_CANCEL        = 20017;      //取消验收
	const MAO_CHECK_DONE          = 20013;      //验收完成
	const MAO_ORDER_CANCEL        = 20006;      //订单申请撤销
	const MAO_ORDER_CANCEL_CANCEL = 20012;      //取消订单撤销
	const MAO_ORDER_CANCEL_OK     = 20009;      //同意订单撤销
	const MAO_ARBITRATION         = 20007;      //申请仲裁
	const MAO_ARBITRATION_CANCEL  = 20008;      //取消仲裁
	const MAO_EXCEPTION           = 20004;      //提交异常
	const MAO_EXCEPTION_CANCEL    = 20011;      //取消异常
	const MAO_ADD_TIME            = 22001;      //补时
	const MAO_ADD_MONEY           = 22002;      //补款
	const MAO_CHANGE_PWD          = 22003;      //修改密码


	private $platform = PlatformAccount::PLATFORM_MAO;
	...
	public function __construct($pt_account_id, $platform_order, $handle_pam = null)
	{
		...
		$this->req    = new MaoReq($pt_account_id);
		$this->syncDetail();
	}
	...
	public function publish()
	{
		...
		if ($result['result'] == 1) {
			$success = '发布到代练猫成功';
			// 更新 订单号
			$this->status->mao_order_no   = $result['data']['orderid'];
			$this->status->mao_pt_message = $success;
			$this->status->mao_is_error   = 0;
			$this->status->mao_is_delete  = 0;
			$this->status->mao_is_publish = 1;
			$this->status->save();
			...
			// 记录同步订单错误
			$this->status->mao_is_delete  = 0;
			$this->status->mao_is_publish = 0;
			$this->status->mao_is_error   = 1;
			$this->status->mao_pt_message = $error;
			$this->status->save();
	...
	public function up()
	{
		$mark = '[上架]';
		if (!$this->req->make('up', [
			'orderid' => $this->status->mao_order_no,
		])
		) {
			return $this->setError($this->req->getError());
		}
		...
		return $this->setError('代练猫服务器返回超出可识别范围');
	}
	...
	public function pubCancel($flag, $pub_pay, $sd_pay, $reason = '')
	{
		...
		switch ($flag) {
			case self::PUB_CANCEL_CANCEL:
				$param = [
					'orderid' => $this->status->mao_order_no,
					'control' => self::MAO_ORDER_CANCEL_CANCEL,
				];
				break;
			case self::PUB_CANCEL_AGREE:
				...
				$param  = [
					'orderid'          => $this->status->mao_order_no,
					'control'          => self::MAO_ORDER_CANCEL_OK,
	...
	public function syncDetail()
	{
		...
		$this->status->mao_left_hour     = (int) array_get($detail, 'data.info.surplustime');
		$this->status->mao_status_desc   = array_get($detail, 'data.info.orderstatusname');
		$this->status->mao_started_at    = array_get($detail, 'data.info.receivetime');
		$this->status->mao_order_hour    = array_get($detail, 'data.info.requiretime');
		$this->status->mao_sd_name       = array_get($detail, 'data.userinfo.nickname');
		$this->status->mao_sd_uid        = array_get($detail, 'data.userinfo.userid');
		$this->status->mao_sd_mobile     = array_get($detail, 'data.userinfo.linktel');
		$this->status->mao_sd_qq         = array_get($detail, 'data.userinfo.qq');
		$this->status->mao_close_day     = (int) array_get($detail, 'data.info.closeday');
		$this->status->mao_can_modify    = (int) array_get($detail, 'data.info.canmodify');
		$this->status->mao_created_at    = array_get($detail, 'data.info.createtime');
		$this->status->mao_type_title    = array_get($detail, 'data.info.dltype');
		$this->status->mao_game_id       = array_get($detail, 'data.info.gameid');
		$this->status->mao_updated_at    = array_get($detail, 'data.info.lastmodifytime');
		$this->status->mao_order_status  = (int) array_get($detail, 'data.info.orderstatus');
		$this->status->mao_review_status = (int) array_get($detail, 'data.info.reviewstatus');
		$this->status->mao_sell_id       = array_get($detail, 'data.info.sellerid');
		$this->status->mao_upload_result = (int) array_get($detail, 'data.info.uploadResult');
		$this->status->mao_order_price   = (int) array_get($detail, 'data.info.unitprice');
		$this->status->sync_at           = Carbon::now();

		if (in_array($this->status->mao_order_status, [self::ORDER_STATUS_ING]) ||
			in_array($this->status->mao_order_status, [self::ORDER_STATUS_EXCEPTION]) ||
			in_array($this->status->mao_order_status, [self::ORDER_STATUS_WAIT_VERIFY]) ||
			in_array($this->status->mao_order_status, [self::ORDER_STATUS_CANCELING]) ||
			in_array($this->status->mao_order_status, [self::ORDER_STATUS_LOCK])
		) {
			// 已经接手
			$this->status->mao_is_accept = 1;
			$this->status->accepted_at   = $this->status->mao_started_at;
		}
		$this->status->save();
	...
	{
		dispatch(new StaticsMaoOrderNum($this->platformAccount, $this->platform));
	}
	...
	private function syncToPlatformOrder()
	{
		...
		// 已经接单, 同步到主订单
		$this->order->assigned_at = $this->status->mao_started_at;
		$this->order->order_hours = $this->status->mao_order_hour;
		$this->order->sd_username = $this->status->mao_sd_name;
		$this->order->sd_uid      = $this->status->mao_sd_uid;
		$this->order->sd_qq       = $this->status->mao_sd_qq;
		$this->order->sd_mobile   = $this->status->mao_sd_mobile;
		$this->order->is_star     = $this->status->mao_review_status;
		$this->order->create_uid  = $this->status->mao_sell_id;
		$this->order->updated_at  = $this->status->mao_updated_at;
		$this->order->order_price = $this->status->mao_order_price;

		switch ($this->status->mao_order_status) {
			...
			case self::ORDER_STATUS_CANCELING :
				...
				if ($this->status->mao_status_desc == '仲裁中') {
	...
	public function modify($type, $value)
	{
		...
		switch ($type) {
			case self::MAO_ADD_TIME:
				...
				break;
			case self::MAO_ADD_MONEY:
				$verify = self::checkTradePwdRDM();
				$param  = [
					'orderid'          => $this->status->mao_order_no,
					'control'          => $type,
					'unitpriceadd'     => $value,
		...
		$result = $this->req->getResp();
		if ($result['result'] == 1) {
			if ($type == self::MAO_CHANGE_PWD) {
				$this->order->game_pwd = $value;
				$this->order->save();
			}
			else if ($type == self::MAO_ADD_MONEY) {
				$this->order->order_price += $value;
				$this->order->save();
			}
			$this->status->mao_pt_message = $result['data'];
			$this->status->mao_is_error   = 0;
			$this->status->save();
			return true;
		}
		else {
			$this->status->mao_pt_message = $result['_message'];
	...
	{
		$pay_password = md5(md5($this->platformAccount->mao_payword) . 'dlmapp');
		$arr          = [
	...
	public function progress($message, $image = null)
	{
		...
		$param = [
			'orderid'     => $this->status->mao_order_no,
			'imgurl'      => $endpoint . $bucket_path . $time . $imageName,
			'description' => $message,
		];
	...
	public function lock($flag, $reason = '')
	{
		if ($flag == self::LOCK_LOCK) {
			$param = [
				'orderid' => $this->status->mao_order_no,
				'control' => Mao::MAO_LOCK,
			];
		}
		else {
			$param = [
				'orderid' => $this->status->mao_order_no,
				'control' => Mao::MAO_UNLOCK,
			];
		}
		...
		if ($result['result'] == 1) {
			...
			if ($flag == self::LOCK_LOCK) {
				...
				$this->status->mao_is_lock      = 1;
				$this->status->mao_order_status = Mao::ORDER_STATUS_LOCK;
			}
			else if ($flag == self::LOCK_UNLOCK) {
				$PlatformOrder->unlock($this->status->order_id, $reason, $this->handlePam->account_id);
				$this->status->mao_is_lock      = 0;
				$this->status->mao_order_status = Mao::ORDER_STATUS_ING;
			}
	...
	public function kf($kf, $reason)
	{
		if ($kf) {
			...
			$param = [
				'orderid' => $this->status->mao_order_no,
				'control' => self::MAO_ARBITRATION_CANCEL,
				'reason'  => $reason,
			];
		}
		$kfDesc = self::kvKf($kf);
