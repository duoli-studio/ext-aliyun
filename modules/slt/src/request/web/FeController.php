...
class FeController extends InitController
{
	...
	public function js($plugin = null)
	{
		...
		foreach ($directories as $dir) {
			...
			elseif (strpos($dir, 'jquery.bt3/') !== false) {
				$dir = str_replace('jquery.bt3/', '', $dir);
				if (strpos($dir, '/') === false) {
					$bt3[] = $dir;
				}
			}
		...
		if (strpos($plugin, 'jquery.bt3') !== false) {
			// bt3
			$viewDir   = 'auto_jquery_bt3';
			$viewName  = substr($plugin, 11);
			$pluginDir = 'assets/js/libs/jquery.bt3/' . $viewName;
		}
		...
		$viewPath    = resource_path('views/web/fe/' . $viewDir . '/' . $viewName . '.blade.php');
		$viewContent = '';
		if (file_exists($viewPath)) {
			$viewContent = view('web.fe.' . $viewDir . '.' . $viewName);
		}
...
HTML;
	...
	public function react($book)
	{
		return view('web.fe.react.' . $book);
	}
}
