...
export default {
	...
	computed : {
		enabled() {
			return this.$store.state.setting['system:debug.enabled'] === '1';
		},
		testing() {
			return this.$store.state.setting['system:debug.testing'] === '1';
		},
	},
	data() {
		return {
	...
	methods  : {
		clearCache() {
			const self = this;
			self.loading = true;
			self.$http.post(`${window.api}backend/system/core/cache_clear`).then(() => {
				self.$notice.open({
					title : '缓存清除成功！',
				});
			}).catch(() => {
		...
		enableDebug(status) {
			const self = this;
			self.$loading.start();
			self.$http.post(`${window.api}backend/system/setting/establish`, {
				key   : 'system::debug.enabled',
			...
			<row>
				<i-col span="12">
					<form-item label="缓存清除">
						<i-button icon="ios-trash" :loading="loading" size="small" type="primary"
