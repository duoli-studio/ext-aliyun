...
export default {
	beforeRouteEnter(to, from, next) {
		...
		injection.http.post(`${window.api}backend/system/mail/fetch`).then(response => {
			next(vm => {
	...
	methods : {
		send() {
			...
			self.$refs.form.validate(valid => {
				if (valid) {
					self.$http.post(`${window.api}backend/system/mail/test`, {
		...
		submit() {
			const self = this;
			self.loading = true;
			injection.http.post(`${window.api}backend/system/mail/store`, self.form).then(() => {
...
<template>
	<card :bordered="false">
		...
		<modal :loading="modal.loading" title="发送测试邮件" :value="modal.visible" @on-cancel="modal.visible = false"
			   @on-ok="send">
