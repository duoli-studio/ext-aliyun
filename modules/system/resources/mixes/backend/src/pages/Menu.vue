...
export default {
	beforeRouteEnter(to, from, next) {
		...
		injection.http.get(`${window.api}backend/system/layout/menus`).then(response => {
			next(vm => {
	...
	methods    : {
		...
		refresh() {
			...
			self.$loading.start();
			self.$http.get(`${window.api}backend/system/layout/menus`).then(response => {
		...
		update() {
			...
			self.$refs.form.validate(valid => {
				if (valid) {
					self.$http.post(`${window.api}backend/system/layout/menus`, {
						data : self.originals,
					}).then(() => {
						self.$notice.open({
							title : '数据批量更新成功！',
												...
												<td>
													<div class="ivu-table-cell">
														<form-item label=""
																   ...
																   style="margin-bottom: 0;">
															<i-switch
																	v-model="originals[item.index].enabled"></i-switch>
														</form-item>
													</div>
												</td>
											</tr>
