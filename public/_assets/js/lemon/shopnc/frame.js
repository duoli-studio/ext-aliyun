...
define(function (require) {
	...
	//iframe location
	if (cookie.get('desktop#route') != null) {
		_openItem(cookie.get('desktop#route'));
	} else {
		$('#J_mainMenu>ul').first().css('display', 'block');
		//第一次进入后台时，默认定到欢迎界面

		var $item = $('a[data-route="' + lemon.url_site + '/dsk_lemon_home/welcome' + '"]');
	...
	if (typeof window.desktop_sitemap != 'undefined' &&
		...
		$("#J_sitemap").on('click', function () {
			var template = handlebars.compile(tpl.sitemap);
			layer.open({
				title  : '网站地图',
				content: template(window.desktop_sitemap)
			});
		});
	} else {
	...
	function _openItem(route, rel) {
		...
		// record
		cookie.set('desktop#route', route);
