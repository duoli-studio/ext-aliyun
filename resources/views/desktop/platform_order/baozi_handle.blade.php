...
@can('baozi_star', $status)
	<button class=" btn btn-primary btn-sm mr3 J_dialog text-info" data-element="#detail_star" data-title="评价"><span>评价</span></button>
@endcan
@can('baozi_progress', $status)
	<a class=" btn btn-primary btn-sm mr3 J_iframe text-info" data-shade_close="true" href="{!! route_url('dsk_platform_status_baozi.progress_item', $status->id) !!}" data-title="更新进度"><span>更新进度</span></a>
@endcan
@can('baozi_over', $status)
	<a class=" btn btn-primary btn-sm mr3 J_request text-danger " data-confirm="确认完成订单?" href="{!! route('dsk_platform_status_baozi.over', [$status->id, $order->order_id]) !!}">
		<span> 确认完单</span>
	</a>
@endcan
...
<div id="detail_lock" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.lock', 'id' => 'form_lock']) !!}
...
<div id="detail_unlock" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.un_lock', 'id' => 'form_unlock']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	<table class="table">
		<tr>
			<td>变更状态</td>
...
<div id="detail_money" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.add_money', 'id' => 'form_money']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	<table class="table">
		<tr>
...
<div id="detail_time" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.add_time', 'id' => 'form_time']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	<table class="table">
		<tr>
...
<div id="detail_game_pwd" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.game_pwd', 'id' => 'form_game_pwd']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	<table class="table">
        <tr>
...
<div id="detail_cancel_apply" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.cancel', 'id'=> 'form_baozi_pub_cancel']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	{!! Form::hidden('flag', \App\Lemon\Dailian\Application\Platform\Baozi::PUB_CANCEL_APPLY) !!}
	<table class="table">
		<tr>
	...
	require(['jquery', 'lemon/util', 'jquery.validate','jquery.form'], function ($, util) {
		...
		$('#form_baozi_pub_cancel').validate(conf);
	});
	</script>
</div>
<div id="detail_sd_cancel_handle" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.cancel']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	<table class="table">
		<tr>
			<td class="w108">撤销标记:</td>
			<td>
				{!! Form::radios('flag', [
				\App\Lemon\Dailian\Application\Platform\Baozi::PUB_CANCEL_AGREE => \App\Lemon\Dailian\Application\Platform\Baozi::kvPubCancel(\App\Lemon\Dailian\Application\Platform\Baozi::PUB_CANCEL_AGREE),
				], \App\Lemon\Dailian\Application\Platform\Baozi::PUB_CANCEL_AGREE, ['desktop' => true]) !!}
			</td>
		</tr>
        <tr>
            <td></td>
		...
		<tr>
			...
			<td>
				<p class="mb5">
					{!! Form::label('order_price', '您已预付的代练费') !!} ¥ {!! $status->baozi_order_price !!} 元
				</p>
				<div class="form-group clearfix mb5 ">
					{!! Form::label('order_safe_money', '需要支付的代练费') !!} ¥ {!! $status->baozi_pub_pay !!} 元
				</div>
				<div class="form-group clearfix mb5">
					{!! Form::label('order_safe_money', '代练员支付的赔偿金') !!} ¥ {!! $status->baozi_sd_pay !!} 元
				</div>
			</td>
		</tr>
		<tr>
...
<div id="detail_cancel_cancel" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.cancel']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	{!! Form::hidden('flag', \App\Lemon\Dailian\Application\Platform\Baozi::PUB_CANCEL_CANCEL) !!}
	<table class="table">
		<tr>
			<td class="w108">撤销标记:</td>
			<td>取消撤销</td>
...
<div id="detail_star" class="hidden">
	{!! Form::open(['route' => 'dsk_platform_status_baozi.star', 'id' => 'form_star']) !!}
	{!! Form::hidden('status_id', $status->id) !!}
	<table class="table">
		<tr>
			<td class="w108">评价:</td>
			<td>{!! Form::radios('flag', \App\Lemon\Dailian\Application\Platform\Baozi::kvStar(), null, [
			  'desktop' => true,
			  'inline' => false,
			]) !!} </td>
		</tr>
